...
Should molecular property prediction models incorporate target context? The answer is not obvious. Per-target models achieve excellent performance when training data is abundant. Multi-task models promise knowledge transfer but risk negative transfer when tasks conflict. Despite widespread interest in context-conditional architectures, \textit{when} and \textit{why} context helps remains poorly characterized.
...
\textbf{Key findings.} (1) \textit{Context fusion matters}: FiLM outperforms concatenation by 24.2 pp and additive conditioning by 8.6 pp. (2) \textit{Context helps most targets}: 9/10 targets improve (mean +5.7 pp, $p < 0.01$). (3) \textit{Context enables data-scarce targets}: On CYP3A4 (67 actives), per-target RF collapses to 0.238 AUC while multi-task transfer achieves 0.686. (4) \textit{Context can hurt}: BACE1 shows $-$10.2 pp due to distribution mismatch.

\textbf{Benchmark critique.} We document severe DUD-E limitations: 1-NN Tanimoto achieves 0.991 AUC without learning; 50\% of actives overlap with ChEMBL training. Our temporal split (train $\leq$2020, test 2021--2024) provides rigorous evaluation, showing stable 0.843 AUC across years.

\section{Problem Setting}
\label{sec:problem}

Let $\mathcal{G} = (V, E, \mathbf{X}^V, \mathbf{X}^E)$ denote a molecular graph with atom set $V$, bond set $E$, atom features $\mathbf{X}^V \in \mathbb{R}^{|V| \times d_v}$, and bond features $\mathbf{X}^E \in \mathbb{R}^{|E| \times d_e}$. We use $\mathcal{C} = \mathcal{P} \times \mathcal{A} \times \mathcal{R}$ to denote the context space (programs $\times$ assays $\times$ rounds). Given context tuple $c = (p, a, r) \in \mathcal{C}$, the model predicts activity $\hat{y} = f(\mathcal{G}, c; \theta)$.

The key challenge is that $P(\mathcal{G}, y | c)$ shifts systematically across contexts. Unlike random covariate shift, this shift is \textit{structured}: early DMTA rounds contain diverse HTS scaffolds; later rounds concentrate on optimized lead series. Standard approaches assume stationarity; we instead learn context-conditional predictors via Feature-wise Linear Modulation (FiLM) \citep{perez_2018_film}:
\begin{equation}
    \film(\mathbf{h}, \mathbf{c}) = \boldsymbol{\gamma}(\mathbf{c}) \odot \mathbf{h} + \boldsymbol{\beta}(\mathbf{c})
\end{equation}
where $\boldsymbol{\gamma}, \boldsymbol{\beta}$ are learned functions producing scale and shift parameters from context $\mathbf{c}$.
...
\methodname{} consists of four components (Figure~\ref{fig:architecture}): (1) an MPNN backbone for molecular encoding, (2) hierarchical context embeddings, (3) FiLM-based modulation, and (4) multi-task prediction heads.

\textbf{Molecular Encoder (L0).} A 6-layer message-passing neural network with GRU-based state updates \citep{li_2016_gru_gnn} transforms molecular graphs into 512-dim representations. Atom features (70-dim) encode chemical properties; bond features (9-dim) encode connectivity.

\textbf{Hierarchical Context Embeddings.} Learnable embedding tables capture context at each level: L1 (target, 128-dim), L2 (assay type, 64-dim), L3 (temporal round, 32-dim). Embeddings are concatenated and projected to 224-dim.

\textbf{FiLM Context Modulation.} The context vector modulates molecular representations via:
$\hmod = \boldsymbol{\gamma}(\mathbf{c}) \odot \hmol + \boldsymbol{\beta}(\mathbf{c})$,
where $\boldsymbol{\gamma}, \boldsymbol{\beta}: \mathbb{R}^{224} \rightarrow \mathbb{R}^{512}$ are two-layer MLPs initialized near identity.

\textbf{Training.} Phase 1 pretrains L0 on ChEMBL 35 \citep{zdrazil_2024_chembl} (21.1M records). Phase 2 fine-tunes with differential learning rates (backbone $10^{-5}$, context $10^{-3}$). Phase 3 performs continual updates during DMTA deployment.

\section{Experiments}
\label{sec:experiments}

We design experiments to answer: \textbf{(Q1)} Does context improve virtual screening? \textbf{(Q2)} Which context levels contribute most? \textbf{(Q3)} Does \methodname{} provide practical value in drug discovery?

\textbf{Setup.} We evaluate on DUD-E \citep{mysinger_2012_dude}, focusing on 10 diverse targets: kinases (EGFR, JAK2), GPCRs (DRD2, ADRB2), nuclear receptors (ESR1, PPARG), proteases (BACE1, FXA), and enzymes (HDAC2, CYP3A4). Baselines include Random Forest with Morgan fingerprints, L0-only (MPNN without context), and GNN-VS \citep{lim_2019_gnn_vs}. We use 5-fold stratified CV with 5 random seeds.

\subsection{DUD-E Results and Benchmark Critique}

\begin{table}[t]
\vspace{-2mm}
\caption{\textbf{DUD-E comparison.} Per-target RF wins on data-rich targets; \methodname{} wins on data-scarce CYP3A4. $^\dagger$Exploits structural bias. $^\ddagger$Trained per-target on ChEMBL.}
\label{tab:sota}
\centering
\small
\begin{tabular}{@{}llcc@{}}
...
Table~\ref{tab:sota} compares \methodname{} to published methods. Per-target RF achieves 0.875 mean AUC, winning on 8/10 targets. This highlights our core finding: context conditioning's value is enabling predictions on data-scarce targets where per-target methods fail, not beating RF on data-rich targets.

\textbf{DUD-E Limitations.} (1) \textit{Structural bias}: 1-NN Tanimoto achieves 0.991 AUC without learning---decoys are trivially distinguishable. (2) \textit{Data leakage}: 50\% of DUD-E actives appear in ChEMBL training. (3) \textit{Per-target RF dominates}: With sufficient data, fingerprint models outperform neural methods.

\begin{table}[t]
\vspace{-2mm}
\caption{Per-target DUD-E results (ROC-AUC). \methodname{} values use correct L1 context.}
...
\vspace{-2mm}
\caption{\textbf{Main Results.} (A) Per-target ROC-AUC. (B) L1 ablation: correct vs.\ generic embeddings (+5.7 pp mean). (C) Model variant comparison. (D) Mean AUC vs.\ prior methods.}
...
Table~\ref{tab:l1_ablation} compares correct target-specific L1 versus generic (zero) embeddings. Target-specific context improves 9/10 targets (mean +5.7 pp, all $p < 0.01$). ESR1 (+13.4 pp) and EGFR (+13.2 pp) benefit most. BACE1 ($-$10.2 pp) reflects distribution mismatch between ChEMBL and DUD-E.

\begin{table}[t]
\vspace{-2mm}
\caption{L1 context ablation (5 seeds, paired $t$-test with Bonferroni correction). $\Delta$ = Correct $-$ Generic.}
...
\subsection{Multi-Task Transfer and Context Fusion}

The strongest evidence for multi-task architectures comes from CYP3A4 (67 training actives): per-target RF achieves 0.238 AUC (worse than random) while \methodname{} achieves 0.686 (2.9$\times$ improvement).

\begin{table}[h]
\vspace{-2mm}
\caption{\textbf{Context fusion comparison.} FiLM outperforms all alternatives. $\Delta$ in percentage points.}
\label{tab:fusion}
\centering
\small
\begin{tabular}{@{}lccc@{}}
...
\subsection{Attribution and Temporal Analysis}

\begin{figure}[t]
\centering
\includegraphics[width=0.88\textwidth]{figures/fig3_attribution.pdf}
\vspace{-2mm}
\caption{\textbf{Attribution Analysis.} (A) Integrated gradients across 5 L1 contexts. (B) Cosine similarity drops from 0.999 (L0-only) to 0.878 (\methodname{}), confirming context-specific explanations.}
\label{fig:attribution}
\vspace{-2mm}
\end{figure}

Using integrated gradients \citep{sundararajan_2017_ig}, we verify context produces different atom-level attributions. Figure~\ref{fig:attribution} shows cosine similarity between attribution vectors drops from 0.999 (L0-only) to 0.878 (\methodname{}).

\textbf{Temporal Split.} Training on ChEMBL data $\leq$2020 and testing on 2021--2024, \methodname{} achieves \textbf{0.843 ROC-AUC} with no degradation across years, avoiding both DUD-E's structural bias and train-test leakage.
...
\vspace{-2mm}
\caption{\textbf{DMTA Replay.} (A) Hit rates: model 75--88\% vs.\ random 40--52\%. (B) Enrichment 1.5--1.9$\times$. (C) 29--55\% fewer experiments. (D) 1.60$\times$ mean enrichment.}
\label{fig:dmta}
\vspace{-2mm}
\end{figure}

Simulating DMTA campaigns using ChEMBL publication dates, \methodname{} achieves \textbf{1.60$\times$ enrichment} (73.4\% vs 45.7\% hit rate), reducing experiments to find 50 hits by 32\%.

\section{Related Work}
\label{sec:related}

\paragraph{Molecular Property Prediction.}
Graph neural networks dominate molecular property prediction following \citet{gilmer_2017_mpnn}, with extensions for attention \citep{xiong_2019_attentivefp}, pretraining \citep{rong_2020_grover, hu_2020_gnn_pretrain}, and 3D geometry \citep{schutt_2017_schnet, gasteiger_2020_dimenet}. Recent molecular foundation models---ChemBERTa \citep{chithrananda_2020_chemberta}, MolBERT \citep{fabian_2020_molbert}, Uni-Mol \citep{zhou_2023_unimol}---achieve strong performance via large-scale pretraining on SMILES or 3D conformers. These methods learn static representations; we introduce context-conditional modulation enabling adaptation based on target, assay, and temporal information. Our approach is orthogonal to foundation models: FiLM conditioning could be applied atop any pretrained encoder.

\paragraph{Conditional Networks.}
FiLM \citep{perez_2018_film} modulates activations via learned scale/shift parameters. We adopt FiLM for its favorable expressiveness-efficiency tradeoff over hypernetworks \citep{ha_2017_hypernetworks}, which generate all network weights from context but require 3$\times$ more parameters for marginal gains.

\paragraph{Distribution Shift.}
Temporal shift in drug discovery is well-documented \citep{sheridan_2013_time_split, martin_2019_temporal}. Domain adaptation \citep{ben_david_2010_theory} learns domain-invariant features; we instead model context to leverage structured shift.
...
Several promising directions emerge from our findings. First, leakage-stratified analysis could isolate transfer learning benefits from memorization effects. Second, evaluation on additional benchmarks (LIT-PCBA, MoleculeACE \citep{van_tilborg_2024_moleculeace}) would test generalization. Third, combining FiLM conditioning with foundation model backbones (Uni-Mol, ChemBERTa) may amplify context benefits. Finally, meta-learning approaches for few-shot target adaptation warrant investigation, as our current few-shot results were negative.

\section{Conclusion}
\label{sec:conclusion}

We presented a systematic study of when target context helps molecular property prediction:
\begin{enumerate}[noitemsep,topsep=2pt]
\item \textbf{When context helps}: 9/10 targets improve (+5.7 pp mean); data-scarce targets benefit most (CYP3A4: 2.9$\times$ vs per-target RF)
...
\item \textbf{Benchmark limitations}: DUD-E is severely compromised; temporal split provides rigorous evaluation
\end{enumerate}

\textbf{Practical guidance}: Use context conditioning when per-target training data is limited (tens to hundreds of compounds), train/test distributions are aligned, and the alternative is a data-scarce per-target model.

\textbf{Limitations.} L2/L3 context showed no benefit due to missing metadata. Foundation model comparison was not performed; FiLM conditioning can be applied atop any encoder.

Code at \url{https://github.com/anonymoussubmitter-167/NEST-DRUG}.

\subsubsection*{Reproducibility Statement}
All datasets are public (ChEMBL, DUD-E, TDC). Hyperparameters in Appendix~\ref{app:hyperparams}.

\subsubsection*{Ethics Statement}
We acknowledge dual-use concerns and mitigate by training only on therapeutic targets. Training required $\sim$200 GPU-hours; we release pretrained models.

\bibliography{references}
\bibliographystyle{iclr2026_conference}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% CHALLENGE SECTION - ICLR Deep Learning Workshop
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\newpage
\section*{Challenge: Science of Deep Learning Improvement}

\subsection*{1. What model or technique are you targeting?}

We target \textbf{Feature-wise Linear Modulation (FiLM)} \citep{perez_2018_film} for context-conditional molecular property prediction. FiLM modulates neural network activations via learned scale ($\gamma$) and shift ($\beta$) parameters derived from auxiliary context:
\begin{equation}
\text{FiLM}(\mathbf{h}, \mathbf{c}) = \boldsymbol{\gamma}(\mathbf{c}) \odot \mathbf{h} + \boldsymbol{\beta}(\mathbf{c})
\end{equation}
The base representation $\mathbf{h}$ comes from an MPNN encoder; context $\mathbf{c}$ encodes target identity, assay type, and temporal information. This represents a broader class of \textbf{conditional computation} techniques where network behavior is modulated by auxiliary information.

We compare FiLM against concatenation, additive conditioning ($\beta$-only), and hypernetworks. FiLM achieves 0.849 AUC with 3$\times$ fewer parameters than hypernetworks (0.841 AUC), establishing it as the most efficient context fusion method.

\subsection*{2. How do your results contribute to understanding these models?}

\textbf{(a) When context helps vs.\ hurts.} We systematically map success and failure modes across 10 protein targets. Context improves 9/10 targets (+5.7 pp mean AUC) but \textit{hurts} BACE1 by 10.2 pp due to distribution mismatch between training (peptidomimetic inhibitors) and evaluation (diverse scaffolds). This reveals that context conditioning requires alignment between training and deployment distributions.

\textbf{(b) FiLM mechanism decomposition.} Through ablation, we isolate multiplicative vs.\ additive contributions: $\gamma$-only achieves 0.831 AUC (69\% of FiLM's benefit over no context); $\beta$-only achieves 0.763 AUC (substantially weaker). This demonstrates that selective feature \textit{amplification} via $\gamma$ is more powerful than bias shifting, suggesting context primarily modulates \textit{which} features are relevant.

\textbf{(c) Data requirements.} Context conditioning provides 2.9$\times$ improvement when per-target data is scarce ($<$100 compounds). With abundant data ($>$1000 compounds), per-target Random Forest remains competitive (0.875 vs.\ 0.850 AUC). This establishes clear decision boundaries for practitioners.

\subsection*{3. How do you expect your submission to influence future work?}

\textbf{(a) Practitioner guidelines.} Our systematic comparison provides concrete guidance for deploying context-conditional architectures: use FiLM over concatenation (+24.2 pp); prefer multiplicative modulation when parameter budget is constrained; verify train/test distribution alignment before deployment to avoid negative transfer.

\textbf{(b) Benchmark methodology.} Our audit exposes severe benchmark limitations (1-NN achieves 0.991 AUC on DUD-E; 50\% active leakage). This ``auditing'' methodology---testing trivial baselines and quantifying data leakage---can be applied to benchmarks in other domains. Our temporal split protocol (train $\leq$2020, test 2021--2024) provides a template for rigorous prospective evaluation.

\textbf{(c) Foundation model integration.} FiLM conditioning is orthogonal to encoder choice. Future work should apply this strategy atop molecular foundation models (Uni-Mol, ChemBERTa), investigating whether pretrained representations amplify or diminish context benefits. Our experimental framework provides the methodology to systematically investigate these interactions.

\textbf{Broader implications.} Context-conditional modulation is most valuable in multi-task settings with heterogeneous data availability across tasks. The principle that multiplicative modulation outperforms additive conditioning may transfer to other domains where task-specific feature relevance varies (e.g., medical imaging across demographics, NLP across languages, recommender systems across user segments).
