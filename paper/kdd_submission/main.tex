%% NEST-DRUG --- KDD / ACM sigconf version, ported from the
%% rebuttal-updated arXiv manuscript (paper/arxiv_submission/main.tex).
%%
%% For double-blind review, switch the documentclass options to
%%     [sigconf, anonymous, review]
%% and add \acmSubmissionID{...} once you have one.

\documentclass[sigconf]{acmart}

%% --- Citation style ---------------------------------------------------------
%% acmart's default is numeric. The original paper used natbib's \citep / \citet;
%% we alias them to \cite so the manuscript compiles without natbib while
%% keeping the source diffs minimal.
\let\citep\cite
\let\citet\cite
...
%% --- ACM rights / venue metadata -------------------------------------------
%% Replace the placeholders below with the values from your ACM rights form.
\setcopyright{acmlicensed}
\copyrightyear{2026}
\acmYear{2026}
\acmDOI{XXXXXXX.XXXXXXX}
\acmConference[KDD '26]{Proceedings of the 32nd ACM SIGKDD Conference on
  Knowledge Discovery and Data Mining}{August 9--13, 2026}{Toronto, ON, Canada}
\acmISBN{978-1-4503-XXXX-X/26/08}

\begin{document}

\title[When Does Context Help? Target-Conditional Molecular Property Prediction]{When Does Context Help?\\
A Systematic Study of Target-Conditional Molecular Property Prediction}
...
\renewcommand{\shortauthors}{Cheng and Zhang}

\begin{abstract}
We present a systematic empirical study of when target context helps molecular property prediction, evaluating context conditioning across 10 diverse protein families, 4 fusion architectures, data regimes spanning 67--9{,}409 training compounds, and both temporal and random evaluation splits. Using \methodname{}, a FiLM-based architecture that conditions molecular representations on target identity, we characterize both success and failure modes. (1) \textit{Inclusion of context is first-order; fusion architecture is second-order}: under matched-budget joint training, all four fusion methods that include context (FiLM, additive, concatenation MLP, gated-concat) are statistically indistinguishable ($\pm 0.3$ pp around 0.740 AUC), while no-context falls 4.7 pp behind. The original 24.2 pp ``FiLM dominates'' gap was \emph{entirely} an artifact of using a frozen, randomly initialized concat projection at inference. We retain FiLM as the recommended default for stable initialization, but withdraw the framing that fusion choice matters more than inclusion. (2) \textit{Context enables otherwise infeasible predictions}: on data-scarce CYP3A4 (67 training actives, ChEMBL), multi-task transfer achieves 0.686 AUC where per-target Random Forest collapses to 0.238; we discuss the necessary caveat that 99\% of CYP3A4 actives at \emph{any} pIC$_{50}$ appear in ChEMBL (\S\ref{sec:discussion}). (3) \textit{Context can systematically hurt under distribution mismatch}: BACE1 shows $-$10.2 pp; few-shot adaptation underperforms zero-shot. (4) \textit{Leakage-stratified re-analysis}: the L1 benefit is statistically indistinguishable between high-leakage ($\geq$50\%) and low-leakage ($<$50\%) DUD-E targets ($p = 0.69$, Welch); in the low-leakage stratum (excluding the BACE1 outlier) the benefit is $+$8.1 pp ($p = 0.007$), so the effect is not driven by leakage. We also document fundamental flaws in standard benchmarking---1-nearest-neighbor Tanimoto achieves 0.991 AUC on DUD-E without any learning, and 50\% of actives leak from training data, so absolute DUD-E numbers should not be over-interpreted. Temporal split evaluation (train $\leq$2020, test 2021--2024) achieves stable 0.843 AUC with no degradation. We frame our findings as a stage-specific empirical map of when target-conditional modeling helps in molecular property prediction, with explicit boundary conditions: practitioners should enable context when training data per target is in the 50--5{,}000 range \emph{and} train/test chemical series overlap (Tanimoto similarity $\geq 0.4$); otherwise generic embeddings are preferable.
\end{abstract}

%% --- CCS concepts (replace via https://dl.acm.org/ccs) ---------------------
\begin{CCSXML}
<ccs2012>
<concept>
...
\ccsdesc[100]{Applied computing~Bioinformatics}

\keywords{knowledge discovery in drug discovery, data mining for molecules,
  scientific pattern discovery, multi-task learning, data-driven drug design,
  scalable molecular prediction, benchmark analysis, temporal data mining,
  distribution shift detection, transfer learning, data regime analysis,
  systematic evaluation methodology, predictive modeling, knowledge transfer,
  large-scale chemical data}
...
We present a systematic study of this question, evaluating context conditioning across (1) 10 diverse protein families, (2) 4 fusion architectures, (3) varying data regimes (67--9{,}409 training compounds), and (4) temporal vs.\ random splits. Using \methodname{}, a FiLM-based architecture~\cite{perez_2018_film} that conditions molecular representations on target identity, we map both success and failure modes of context conditioning.

\textbf{Key findings.} (1) \textit{Context inclusion is first-order; fusion architecture is second-order}: under \emph{matched-budget} joint training (\S\ref{sec:fair_fusion}), FiLM, concat-MLP, additive, and gated-concat are all statistically indistinguishable ($\pm 0.3$ pp around 0.740 AUC); the original 24.2 pp FiLM advantage was an artifact of using a frozen, randomly initialized concat projection at inference. The first-order finding is that \emph{including} context yields $+4.7$ pp over no-context. (2) \textit{Context helps most targets}: 9/10 targets improve with target-specific embeddings (mean +5.7 pp, $p < 0.01$). (3) \textit{The L1 benefit is not driven by leakage}: in low-leakage targets (\textless 50\% ChEMBL/DUD-E overlap, excluding BACE1), the mean benefit is $+8.1$ pp ($p = 0.007$); high-vs-low strata are statistically indistinguishable ($p = 0.69$, Welch). (4) \textit{Context enables data-scarce targets}: On CYP3A4 (67 training actives at pIC$_{50}\geq 6$), per-target RF collapses to 0.238 AUC while multi-task transfer achieves 0.686. (5) \textit{Context can hurt}: BACE1 shows $-$10.2 pp under distribution mismatch; few-shot adaptation degrades vs.\ zero-shot.
...
Our contributions: \textbf{(1)} systematic taxonomy of when context helps vs.\ hurts in molecular property prediction; \textbf{(2)} matched-budget fusion comparison showing that, under fair training conditions, FiLM, concat-MLP, additive, and gated-concat are within 0.3 pp of each other---fusion choice is second-order; \textbf{(3)} a leakage-stratified re-analysis showing the L1 benefit holds in low-leakage targets and is not an artifact of train/test contamination; \textbf{(4)} an ex-ante decision rule for when to enable conditioning, based on training-set features computable before any test-set access; \textbf{(5)} characterization of data requirements and distribution alignment for effective context; \textbf{(6)} benchmark audit documenting severe DUD-E limitations---1-NN Tanimoto achieves 0.991 AUC without learning, 50\% of actives leak from training, and cross-target RF achieves 0.746 AUC---with recommendations for temporal splits, scaffold splits, and leakage-stratified reporting.
...
The key challenge is that the joint distribution $P(\mathcal{G}, y \mid c)$ shifts systematically across contexts. Unlike random covariate shift, this shift is \textit{structured} in ways that reflect the drug discovery process:
...
Standard approaches assume stationarity and learn context-agnostic predictors $f(\mathcal{G}; \theta)$. We instead learn context-conditional predictors via Feature-wise Linear Modulation (FiLM)~\cite{perez_2018_film}:
\begin{equation}
    \film(\mathbf{h}, \mathbf{c}) = \boldsymbol{\gamma}(\mathbf{c}) \odot \mathbf{h} + \boldsymbol{\beta}(\mathbf{c})
\end{equation}
where $\boldsymbol{\gamma}, \boldsymbol{\beta}$ are learned functions producing scale and shift parameters from context $\mathbf{c}$. This enables the model to adapt its molecular representation based on the discovery setting. We build on message-passing neural networks (MPNNs)~\cite{gilmer_2017_mpnn} for the base molecular encoder.
...
\Description{Block diagram showing message-passing graph neural network producing a 512-dimensional molecular embedding, hierarchical context embeddings for target, assay, and temporal round combined into a context vector, FiLM modulation applied to the molecular embedding, and multi-task prediction heads for activity and ADMET endpoints.}
...
The L0 backbone transforms molecular graphs into fixed-dimensional representations using a 6-layer message-passing neural network with GRU-based state updates~\cite{li_2016_gru_gnn}. At each layer $t$, atom representations $\mathbf{h}_v^{(t)}$ are updated by aggregating messages from neighbors:
\begin{equation}
...
\begin{equation}
    \hmol = [\text{MeanPool}(\{\mathbf{h}_v^{(T)}\}_{v \in V}) \,\Vert\, \text{MaxPool}(\{\mathbf{h}_v^{(T)}\}_{v \in V})] \in \mathbb{R}^{512}
\end{equation}
...
\begin{equation}
    \mathbf{c} = W_c [\mathbf{e}_p^{(L1)} \,\Vert\, \mathbf{e}_a^{(L2)} \,\Vert\, \mathbf{e}_r^{(L3)}] + \mathbf{b}_c \in \mathbb{R}^{224}
\end{equation}
...
We evaluate on DUD-E~\cite{mysinger_2012_dude}, focusing on 10 diverse targets: kinases (EGFR, JAK2), GPCRs (DRD2, ADRB2), nuclear receptors (ESR1, PPARG), proteases (BACE1, FXA), and enzymes (HDAC2, CYP3A4). Each target contains 200--600 actives with property-matched decoys (1:50 ratio). Pretraining uses ChEMBL 35~\cite{zdrazil_2024_chembl} (21.1M records, 5{,}123 targets). Baselines include Random Forest with Morgan fingerprints, L0-only (MPNN without context), and GNN-VS~\cite{lim_2019_gnn_vs}. We use 5-fold stratified CV with 5 random seeds (25 runs per experiment); statistical significance via paired $t$-tests with Bonferroni correction. Throughout, we report differences in percentage points (pp) rather than relative percentages to avoid ambiguity. See Appendix~\ref{app:hyperparams} for details.
...
\midrule
AtomNet~\cite{wallach_2015_atomnet} & 3D CNN & 0.818 & --- \\
GNN-VS~\cite{lim_2019_gnn_vs} & GNN & 0.825 & --- \\
3D-CNN~\cite{ragoza_2017_3dcnn} & 3D CNN & 0.830 & --- \\
...
\item \textbf{Structural bias}: 1-NN Tanimoto achieves 0.991 AUC without any learning---decoys are trivially distinguishable by structure alone~\cite{wallach_2018_dude_bias}
...
\begin{table}[t]
\caption{Per-target DUD-E results (ROC-AUC). \methodname{} values use correct L1 context. ``DUD-E RF'' = standard RF baseline trained on DUD-E train split only; differs from ChEMBL-trained ``Per-target RF'' in Tables~\ref{tab:sota} and \ref{tab:per_target_rf} (0.875 AUC).}
...
\Description{Four-panel results figure: (A) bar chart of per-target ROC-AUC for NestDrug versus baselines across 10 DUD-E targets; (B) ablation comparing correct versus generic L1 embeddings; (C) model-variant ablation showing the gain from adding L1 context to the L0 backbone; (D) bar chart comparing mean AUC across published DUD-E methods.}
...
\label{sec:fair_fusion}

Table~\ref{tab:fusion} compares FiLM against alternative context fusion strategies. We report two regimes that bracket the question raised by reviewers: a \emph{frozen-projection} concatenation baseline (post-hoc projection applied to a FiLM-trained backbone, the original Table~\ref{tab:fusion} values) and a \emph{matched-budget} concatenation baseline (concatenation projection trained jointly with backbone and prediction head from scratch, on identical data).

\begin{table}[t]
\caption{\textbf{Context fusion comparison, two regimes.} \textit{Top}: original DUD-E numbers using a frozen post-hoc projection for concatenation. \textit{Bottom}: matched-budget joint training on a temporal ChEMBL split (60K train, 6K val, 3 epochs, 2 seeds; \S\ref{sec:fair_fusion_setup}, Appendix~\ref{app:fair_fusion}). \textbf{Under matched training, all fusion methods that include context are statistically indistinguishable} ($\pm 0.3$ pp around $0.740$); the original 24.2 pp FiLM advantage was \emph{entirely} an artifact of the frozen-projection concat baseline. The first-order finding is therefore inclusion of context ($+$4.7 pp), not the choice of fusion. $\Delta$ in pp.}
\label{tab:fusion}
\centering
\small
\begin{tabular}{@{}lccc@{}}
...
\midrule
\multicolumn{4}{l}{\textit{Original DUD-E (frozen-projection concat baseline)}} \\
Concatenation (frozen proj.)$^\dagger$ & 0.607 & $-$24.2 pp & 0/10 \\
...
\midrule
\multicolumn{4}{l}{\textit{Matched-budget joint training, ChEMBL split (Appendix~\ref{app:fair_fusion})}} \\
No Context & 0.6926 $\pm$ 0.0032 & $-$4.7 pp & --- \\
Additive ($\beta$ only, joint) & 0.7391 $\pm$ 0.0005 & $-$0.05 pp & --- \\
Concatenation (joint, MLP) & 0.7399 $\pm$ 0.0014 & $+$0.03 pp & --- \\
\textbf{FiLM (joint)} & \textbf{0.7396 $\pm$ 0.0011} & \textbf{---} & --- \\
Gated Concat (gate $+$ proj., joint) & 0.7419 $\pm$ 0.0001 & $+$0.23 pp & --- \\
\bottomrule
\end{tabular}
\end{table}

\paragraph{Revised interpretation (vs.\ original draft).} Under matched-budget training, \emph{all} fusion methods that include context perform within 0.3 pp of each other ($0.7391$--$0.7419$), and a gated-concat variant slightly outperforms FiLM. The original 24.2 pp FiLM advantage was \emph{entirely} an artifact of using a frozen, randomly-initialized concatenation projection at inference; the comparison was unfair. We have correspondingly revised our headline claim: \textbf{the first-order finding is inclusion of context ($+$4.7 pp over no context), not the specific fusion architecture}. We retain FiLM as a reasonable default because it is the simplest fusion exposing a multiplicative pathway, has stable initialization (identity at $t = 0$), and matches the gated-concat variant within noise; but we explicitly retract the prior claim that FiLM \emph{dominates} concatenation.

\paragraph{Setup for matched-budget comparison.}
\label{sec:fair_fusion_setup}
We freshly train identical MPNN backbones (4 layers, 256 hidden) with each fusion head from scratch on a stratified 60K-record ChEMBL subset spanning the 10 DUD-E targets and 45 additional programs. All variants use AdamW ($3\!\times\!10^{-4}$), cosine LR decay, 3 epochs, identical batches, and 2 seeds. Mean AUC is the validation AUC computed across all targets relative to the per-target median pIC$_{50}$. Full numbers in Appendix~\ref{app:fair_fusion}.
...
Using integrated gradients~\cite{sundararajan_2017_ig}, we verify context produces different atom-level attributions. Figure~\ref{fig:attribution} shows \textbf{cosine similarity between attribution vectors drops from 0.999 (L0-only) to 0.878 (\methodname{})}---a 12\% reduction confirming context-specific feature weighting.

\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{figures/fig3_attribution.pdf}
...
\Description{Two-panel attribution figure. Left: heatmap of per-atom integrated-gradient importance for the molecule Celecoxib evaluated under five different target contexts; the highlighted atoms differ between contexts. Right: bar chart comparing mean cosine similarity between attribution vectors with and without target context.}
\label{fig:attribution}
\end{figure}

\subsection{Leakage-Stratified Re-Analysis of L1 Benefit}
\label{sec:leakage_strat}

DUD-E--ChEMBL leakage (mean 50\% of actives) could confound any claim that ``context helps,'' since the L1 benefit could simply reflect the model retrieving training-set neighbors. We test this directly by stratifying the L1 benefit on per-target leakage fraction (Table~\ref{tab:leakage_strat}).

\begin{table}[t]
\caption{\textbf{L1 benefit stratified by ChEMBL--DUD-E leakage.} HIGH = $\geq$50\% active leakage; LOW = $<$50\%. The per-target $\Delta$ AUC values are taken from Table~\ref{tab:l1_ablation}.}
\label{tab:leakage_strat}
\centering
\small
\begin{tabular}{@{}lccc@{}}
\toprule
Stratum & Targets & Mean $\Delta$ AUC & $p$ ($\Delta\!=\!0$) \\
\midrule
HIGH leakage ($\geq 50$\%) & CYP3A4, EGFR, FXA, DRD2 & $+0.068 \pm 0.044$ & --- \\
LOW leakage ($< 50$\%, all) & ADRB2, BACE1, ESR1, HDAC2, JAK2, PPARG & $+0.051 \pm 0.074$ & --- \\
LOW leakage, w/o BACE1 & ADRB2, ESR1, HDAC2, JAK2, PPARG & $\mathbf{+0.081 \pm 0.032}$ & $\mathbf{0.007}$ \\
\bottomrule
\end{tabular}
\end{table}

\textbf{Findings.} (1) The HIGH and LOW leakage strata are statistically indistinguishable in mean $\Delta$ (Welch $t$-test $p = 0.69$). (2) Pearson correlation between target-level leakage \% and $\Delta$ AUC is $r = 0.19$ ($p = 0.60$); excluding the BACE1 outlier, $r = -0.13$ ($p = 0.74$). (3) In the LOW-leakage stratum (excluding BACE1), the L1 benefit is $+8.1$ pp ($p = 0.007$, one-sample $t$-test against zero). Together these results show the L1 benefit is \emph{not} explained by training-set leakage; targets with negligible overlap (PPARG: 1.2\%, ADRB2: 3.8\%) show benefits comparable to high-overlap targets. This addresses the central reviewer concern that DUD-E artifacts confound the headline claim.

\subsection{Ex-Ante Decision Rule for Enabling Context}
\label{sec:decision_rule}

``Context helps when distributions align'' is post-hoc and not actionable on its own. We propose a pre-computable decision procedure that uses only \emph{training-set} statistics and a Tanimoto similarity diagnostic against unlabeled candidates.

\paragraph{Decision rule.} Enable target-conditional L1 modulation when:
\begin{itemize}[noitemsep,topsep=0pt]
\item \textbf{Data scale check:} the target has 50--5{,}000 ChEMBL training compounds at the active threshold (above 5{,}000, per-target RF is hard to beat; below 50, the L1 embedding is undertrained); AND
\item \textbf{Distribution alignment check:} the median active--candidate Tanimoto similarity ($k$-NN, $k = 5$, Morgan-1024) between ChEMBL training actives and the candidate set exceeds 0.4. We use the candidate set rather than the test set (which is by definition unknown at deploy time); if a candidate library is unavailable, use the latest 10\% of ChEMBL records for that target as a held-out diagnostic set.
\end{itemize}

\paragraph{Validation.} On our 10-target panel, this rule correctly classifies 9/10 targets (it disables context on BACE1, the single failure case), agreeing with the empirical $\Delta$ AUC sign. The two checks are complementary: data-scale alone has Pearson correlation $r = -0.01$ with $\Delta$ AUC ($p = 0.97$), so no single feature predicts whether context helps---both checks are needed. We provide the diagnostic implementation in Appendix~\ref{app:decision_rule}.

\subsection{Temporal Split Evaluation}
\label{sec:temporal}

Given DUD-E's limitations, we provide temporal split evaluation as stronger evidence. Training on ChEMBL data $\leq$2020 and testing on 2021--2024, \methodname{} achieves \textbf{0.843 ROC-AUC} with no degradation across years (Table~\ref{tab:temporal_main}). This prospective evaluation avoids both DUD-E's structural bias and train-test leakage. We also report a Murcko-scaffold split (Appendix~\ref{app:scaffold}) which preserves stronger generalization claims by removing scaffold continuity that temporal splits can leave intact.
...
\Description{Four-panel DMTA replay figure showing per-target hit rate, enrichment factor, reduction in number of experiments to reach 50 hits, and overall mean enrichment for NestDrug compared with a random baseline.}
\label{fig:dmta}
\end{figure*}
...
Graph neural networks dominate molecular property prediction following \cite{gilmer_2017_mpnn}, with extensions for attention~\cite{xiong_2020_attentivefp}, pretraining~\cite{rong_2020_grover, hu_2020_gnn_pretrain}, and 3D geometry~\cite{schutt_2017_schnet, gasteiger_2020_dimenet}. Recent molecular foundation models---ChemBERTa~\cite{chithrananda_2020_chemberta}, MolBERT~\cite{fabian_2020_molbert}, Uni-Mol~\cite{zhou_2023_unimol}---achieve strong performance via large-scale pretraining on SMILES or 3D conformers. These methods learn static representations; we introduce context-conditional modulation enabling adaptation based on target, assay, and temporal information. Our approach is orthogonal to foundation models: FiLM conditioning could be applied atop any pretrained encoder.

\paragraph{Distribution Shift.}
Temporal shift is well-documented~\cite{sheridan_2013_time_split, martin_2019_temporal}. Domain adaptation~\cite{ben_david_2010_theory} learns domain-invariant features; we instead model context to leverage structured shift.

\paragraph{Multi-Task Learning.}
Multi-task learning improves data-scarce endpoints~\cite{ramsundar_2015_massively}. Our context embeddings modulate shared representations rather than requiring explicit task separation.

\paragraph{Conditional Networks.}
FiLM~\cite{perez_2018_film} modulates activations via learned scale/shift. We adopt FiLM for its favorable expressiveness-efficiency tradeoff over hypernetworks~\cite{ha_2017_hypernetworks}.

\section{Discussion}
\label{sec:discussion}

\paragraph{When to Use Context Conditioning.}
Combining \S\ref{sec:fair_fusion}, \S\ref{sec:leakage_strat}, and \S\ref{sec:decision_rule}: context conditioning is valuable when (1) per-target training data is in the 50--5{,}000 range, (2) median candidate--training Tanimoto similarity exceeds 0.4, and (3) the fusion is jointly trained---FiLM, gated-concat, additive, or a properly trained concat-MLP are all indistinguishable within $\pm 0.3$ pp under matched conditions, so the practical question reduces to ``include context or not'' rather than ``which fusion.'' When data is abundant and distributions match, per-target RF remains highly competitive.

\paragraph{What L1 Embeddings Learn.}
We state this carefully: L1 does \emph{not} appear to learn transferable protein biology---zero-shot transfer to held-out targets shows $\Delta = 0$ (Appendix~\ref{app:extended}), few-shot adaptation degrades by 3--18 pp (Table~\ref{tab:fewshot}), and the learned L1 similarity has near-zero correlation with ESM-2 protein embedding similarity ($r = 0.11$, $p = 0.49$). What L1 \emph{does} appear to learn is target-specific dataset structure: activity-distribution shape, assay-protocol biases, and the chemical series in training data. The accurate framing is therefore that L1 conditioning helps the model adapt to a specific training distribution, not that it captures universally transferable target-conditioned biology. We have updated the title and abstract phrasing accordingly.

\paragraph{What the CYP3A4 Result Does and Does Not Show.}
CYP3A4 requires careful framing. 99\% of DUD-E CYP3A4 actives appear \emph{somewhere} in ChEMBL---but only 67 of those records meet the active threshold (pIC$_{50} \geq 6.0$) used by per-target RF. Per-target RF fails not because the test set is novel but because 67 examples are insufficient regardless of test-set overlap. \methodname{} succeeds via multi-task transfer that pools information across the other 5{,}122 ChEMBL programs. The claim is therefore: \textit{multi-task context conditioning unlocks viable predictions for data-scarce targets in a low-shot regime}, not that the model generalizes to chemical space outside ChEMBL.

\paragraph{Benchmark Recommendations.}
DUD-E's severe limitations (0.991 1-NN AUC, 50\% leakage) make it unsuitable for method comparison. We recommend: (1) temporal splits as standard practice, (2) leakage-stratified reporting, (3) evaluation on structure-balanced benchmarks like LIT-PCBA~\cite{tran_2020_litpcba}.
...
\item \textbf{How to fuse context}: Under matched-budget joint training, FiLM, additive, concat-MLP, and gated-concat are all within $\pm 0.3$ pp; the original 24.2 pp ``FiLM dominates concat'' gap was an inference-time artifact.
...
\paragraph{Limitations.} (i) \textbf{L2/L3 are aspirational, not validated.} L2 (assay) and L3 (temporal) context showed no benefit on ChEMBL because assay\_type is sparsely populated and round\_id was effectively zero. We now frame the hierarchical-context proposal as a methodological design rather than an empirically validated contribution; only L1 is supported by current evidence. Future work on properly annotated proprietary data is required to test L2/L3. (ii) \textbf{CYP3A4 framing.} 99\% of DUD-E actives appear \textit{somewhere} in ChEMBL, but per-target RF trains only on compounds meeting the active threshold (pIC$_{50} \geq 6.0$)---just 67 of 5{,}504 total records. The ``leakage'' reflects presence at \textit{any} activity level. Per-target RF fails because 67 training examples are insufficient regardless of overlap; \methodname{} succeeds via multi-task transfer. (iii) \textbf{Few-shot adaptation degrades.} Our few-shot results suggest that L1 embeddings require thousands of examples to calibrate; meta-learning or protein-language-model initialization is the natural next step. (iv) \textbf{Foundation-model comparison.} We did not benchmark against molecular foundation models (Uni-Mol~\cite{zhou_2023_unimol}, ChemBERTa~\cite{chithrananda_2020_chemberta}). Our contribution is orthogonal---FiLM conditioning can be applied atop any encoder; we discuss a ChemBERTa+FiLM design in Appendix~\ref{app:chemberta} and find context still produces a measurable benefit, but a full evaluation requires resources beyond our scope. (v) \textbf{Memory scaling.} Maintaining program-level embeddings scales linearly with $|\mathcal{P}|$. For 10{,}000+ programs, hashed embeddings or low-rank factorization should be evaluated. (vi) \textbf{External generalization.} Context can hurt on truly external benchmarks (TDC ADMET); embeddings overfit to training-distribution patterns rather than transferable biology.

\paragraph{Future Work.} (1) Leakage-stratified analysis to isolate transfer vs memorization; (2) evaluation on LIT-PCBA~\cite{tran_2020_litpcba} or MoleculeACE~\cite{van_tilborg_2024_moleculeace}; (3) FiLM conditioning with foundation model backbones (Uni-Mol, ChemBERTa); (4) meta-learning for few-shot; (5) proprietary data with L2/L3 metadata.
...
\begin{figure}[h]
\centering
\includegraphics[width=\columnwidth]{figures/fig_appendix.pdf}
...
\Description{Two-panel figure showing per-target delta-AUC for the L2 (assay-type) and L3 (temporal round) context ablations; both panels show small, non-significant effects centered near zero.}
\label{fig:appendix}
\end{figure}
...
\begin{equation}
    D_{\text{KL}}\left( P(\mathcal{G}, y \mid c) \,\|\, P(\mathcal{G}, y \mid c') \right) = \Omega(\|c - c'\|)
\end{equation}
...
\begin{equation}
    \mathbb{E}_{c \sim P(\mathcal{C})} \left[ \mathcal{L}(\hat{f}_{\text{ctx}}, c) \right] \leq \mathbb{E}_{c \sim P(\mathcal{C})} \left[ \mathcal{L}(\hat{f}_{\text{static}}, c) \right] - \Omega\left( \mathrm{Var}_c[P(\mathcal{G}, y \mid c)] \right)
\end{equation}
...
\begin{equation}
    P(y \mid \mathcal{G}, p, a, r) = P(y \mid \mathcal{G}, p) \cdot P(a \mid p) \cdot P(r \mid p, a)
\end{equation}
then the optimal context embedding satisfies $\mathbf{c}^* = f_1(\mathbf{e}_p) + f_2(\mathbf{e}_a \mid \mathbf{e}_p) + f_3(\mathbf{e}_r \mid \mathbf{e}_p, \mathbf{e}_a)$ for some functions $f_1, f_2, f_3$.
\end{proposition}
...
\textbf{Implication}: For truly novel targets, use generic L1 (zero-shot) rather than attempting to adapt. True zero-shot transfer would require protein structure-informed initialization (e.g., ESM-2 embeddings~\cite{lin_2023_esm2}), which we leave for future work.
...
\paragraph{Molecular Featurization.} We convert SMILES strings to molecular graphs using RDKit~\cite{landrum_rdkit}. Atom features (70-dim) include:
...
Let $f^*: \mathcal{G} \times \mathcal{C} \rightarrow \mathbb{R}$ be a Lipschitz-continuous target function with constant $L$. By the universal approximation theorem for MPNNs~\cite{xu_2019_gnn_power}, there exists an MPNN $\phi$ such that $\|\phi(\mathcal{G}) - \phi^*(\mathcal{G})\|_2 < \delta$ for any continuous $\phi^*$ and $\delta > 0$.
...
\begin{proof}
Let $P_c = P(\mathcal{G}, y \mid c)$ denote the context-conditional distribution. The optimal context-conditional predictor is:
\begin{equation}
    \hat{f}_{\text{ctx}}(\mathcal{G}, c) = \mathbb{E}_{y \sim P_c}[y \mid \mathcal{G}]
\end{equation}

The optimal context-agnostic predictor is:
\begin{equation}
    \hat{f}_{\text{static}}(\mathcal{G}) = \mathbb{E}_{c \sim P(\mathcal{C})} \mathbb{E}_{y \sim P_c}[y \mid \mathcal{G}]
\end{equation}

For squared loss, the expected loss of the context-conditional predictor is:
\begin{equation}
    \mathbb{E}_{c, \mathcal{G}, y}[(y - \hat{f}_{\text{ctx}}(\mathcal{G}, c))^2] = \mathbb{E}_{c, \mathcal{G}}[\mathrm{Var}(y \mid \mathcal{G}, c)]
\end{equation}

The expected loss of the context-agnostic predictor is:
\begin{align}
    \mathbb{E}_{c, \mathcal{G}, y}[(y - \hat{f}_{\text{static}}(\mathcal{G}))^2] &= \mathbb{E}_{c, \mathcal{G}}[\mathrm{Var}(y \mid \mathcal{G}, c)] \\
    &\quad + \mathbb{E}_{\mathcal{G}}[\mathrm{Var}_c(\mathbb{E}[y \mid \mathcal{G}, c])]
\end{align}

by the law of total variance. The second term is the excess risk due to ignoring context, which is $\Omega(\mathrm{Var}_c[P_c])$ under structured shift.
\end{proof}
...
We provide additional analysis of DUD-E benchmark characteristics, addressing known limitations in the literature~\cite{wallach_2018_dude_bias}.
...
This confirms~\cite{wallach_2018_dude_bias}: DUD-E decoys are property-matched but not structure-matched, making fingerprint methods trivially effective. Our deep learning SOTA claim remains valid because neural methods cannot exploit these structural shortcuts---they must learn generalizable molecular representations.
...
We tested whether pretrained protein embeddings (ESM-2~\cite{lin_2023_esm2}, 650M parameters) can replace learned L1:
...
\section{Matched-Budget Fair-Fusion Experiment}
\label{app:fair_fusion}

This appendix provides the full setup and per-fusion numbers behind Table~\ref{tab:fusion} (matched-budget regime), which addresses the concern that the original 24.2 pp ``FiLM $>$ concatenation'' gap was inflated by an underspecified concatenation baseline.

\paragraph{Setup.} We train identical 4-layer MPNN backbones (256 hidden, 0.1 dropout) from scratch with five fusion heads:
\begin{itemize}[noitemsep,topsep=0pt]
\item \textbf{None} (no context, L0 only).
\item \textbf{Concat (joint MLP)}: $h_{\text{mod}} = \text{MLP}([h \,\Vert\, c])$ with a 2-layer projection trained jointly with backbone and predictor.
\item \textbf{Additive}: $h_{\text{mod}} = h + \beta(c)$, $\beta$ a 2-layer MLP, jointly trained.
\item \textbf{Gated Concat}: $h_{\text{mod}} = h \odot \sigma(W[h\,\Vert\,c]) + W'[h\,\Vert\,c]$, gate $+$ projection both jointly trained. Equal nominal capacity to FiLM.
\item \textbf{FiLM (joint)}: $h_{\text{mod}} = \gamma(c) \odot h + \beta(c)$, $\gamma$ initialized to $1$ and $\beta$ to $0$.
\end{itemize}
Training uses 60K stratified ChEMBL records covering the 10 DUD-E targets plus 45 additional programs, AdamW with $3\times 10^{-4}$ LR, cosine decay, batch size 256, 3 epochs, 2 seeds. Validation uses 6K held-out records; AUC is computed against the per-target median pIC$_{50}$ as a binarization threshold.

\paragraph{Results.} Mean validation AUC and RMSE across 2 seeds:

\begin{table}[h]
\caption{Matched-budget fusion comparison (mean $\pm$ std over 2 seeds, 60K records, 3 epochs). All four context-fusion methods are statistically indistinguishable; a gated-concat variant slightly outperforms FiLM. The 24.2 pp FiLM advantage in the original Table~\ref{tab:fusion} was entirely an artifact of using a frozen, randomly initialized concat projection at inference.}
\label{tab:fair_fusion}
\centering
\small
\begin{tabular}{@{}lcc@{}}
\toprule
Fusion Method & Mean AUC & RMSE \\
\midrule
None (L0 only) & 0.6926 $\pm$ 0.0032 & 1.197 $\pm$ 0.003 \\
Additive ($\beta$ only, joint) & 0.7391 $\pm$ 0.0005 & 1.132 $\pm$ 0.001 \\
Concatenation (joint MLP) & 0.7399 $\pm$ 0.0014 & 1.129 $\pm$ 0.003 \\
\textbf{FiLM (joint)} & \textbf{0.7396 $\pm$ 0.0011} & 1.128 $\pm$ 0.002 \\
Gated Concat (joint) & \textbf{0.7419 $\pm$ 0.0001} & \textbf{1.124 $\pm$ 0.002} \\
\bottomrule
\end{tabular}
\end{table}

\paragraph{Takeaway.} The concerns about the concatenation baseline were correct, and stronger than originally stated: under matched joint training, the FiLM advantage over concat \emph{disappears entirely} ($+$0.03 pp over concat, $-$0.23 pp vs.\ gated-concat). We have correspondingly retracted the claim that ``how to fuse context matters more than whether to include it.'' Two findings remain robust:
\begin{enumerate}[noitemsep,topsep=1pt]
\item \textbf{Context inclusion is first-order:} adding any of the four fusion methods produces a $+$4.65--4.93 pp gain over no-context.
\item \textbf{Fusion architecture is second-order:} all four context-fusion methods cluster within 0.3 pp of each other.
\end{enumerate}
We recommend FiLM as a default not because of dominant performance, but for two architectural reasons: (i) it has a clean identity initialization ($\gamma$ = 1, $\beta$ = 0), so adding context cannot hurt training stability; (ii) it adds 12\% fewer parameters than a gated-concat variant of equal expressiveness. Practitioners with a tuned concatenation MLP need not switch.

\section{Ex-Ante Decision Rule: Implementation}
\label{app:decision_rule}

The decision rule from \S\ref{sec:decision_rule} can be implemented in $\sim$50 lines of Python and runs in seconds per target:

\begin{quote}
\textbf{Ex-ante decision rule (enable target-conditional L1 modulation?)}
\begin{enumerate}[noitemsep,topsep=1pt,leftmargin=*]
\item \textbf{Inputs:} target $p$; ChEMBL training pool $D_{\text{train}}^{(p)}$; candidate library $C^{(p)}$. Thresholds: $n_{\min}=50$, $n_{\max}=5000$, $\tau_{\text{sim}}=0.4$.
\item \textbf{Data scale check.} Compute $n_{\text{actives}} = |\{x \in D_{\text{train}}^{(p)} : \text{pIC}_{50}(x) \geq 6\}|$. If $n_{\text{actives}} < n_{\min}$ or $n_{\text{actives}} > n_{\max}$, return \texttt{disable} (data scale unfavourable).
\item \textbf{Distribution alignment check.} Compute Morgan-1024 fingerprints for $D_{\text{train}}^{(p)}$ actives and $C^{(p)}$, then $\bar{s} = \mathrm{median}_{c \in C^{(p)}} \mathrm{Tanimoto}_{k=5}(c, D_{\text{train}}^{(p)})$. If $\bar{s} < \tau_{\text{sim}}$, return \texttt{disable} (distribution mismatch).
\item Otherwise return \texttt{enable}.
\end{enumerate}
\end{quote}

\paragraph{Validation.} We compute the rule on the 10-target panel using ChEMBL train and DUD-E candidates as the proxy for the deploy library. The rule disables context on BACE1 (median active--decoy similarity 0.30, below the 0.4 threshold) and enables it on the other 9 targets. This matches the empirical $\Delta$-AUC sign on 9/10 targets. The rule does \emph{not} predict the magnitude of $\Delta$ AUC well (LOO regression $R^2$ is negative), but the binary enable/disable decision is the actionable quantity.

\paragraph{Limitations of the rule.} (1) The 0.4 threshold was tuned on a single 10-target panel and should be re-validated for new chemical-series coverage. (2) The rule does not detect novel-active scaffolds that are unlikely to resemble training actives but are still ``correct''---a candidate library that contains genuinely novel chemotypes will be flagged ``disable'' even if those candidates are real. The rule is conservative by design.

\section{Scaffold-Split Robustness Check}
\label{app:scaffold}

Temporal splits can preserve scaffold continuity and inflate generalization claims. We additionally evaluate matched models (no-context vs.\ FiLM with L1, identical training setup) on a Murcko-scaffold split: 10\% of unique Murcko scaffolds (1{,}769 of 17{,}695) and all molecules sharing those scaffolds are held out from training. The split is computed on the 30K-record stratified ChEMBL subset described in Appendix~\ref{app:fair_fusion}.

\begin{table}[h]
\caption{Scaffold-split evaluation, matched-model comparison (3 epochs, seed 0). The FiLM benefit persists \emph{and increases} under scaffold split, contrary to the worry that the random-split benefit reflects scaffold continuity. FiLM AUC is essentially unchanged across splits (0.7229 vs.\ 0.7236).}
\label{tab:scaffold}
\centering
\small
\begin{tabular}{@{}lccc@{}}
\toprule
Split & No Context & FiLM L1 & $\Delta$ FiLM--None \\
\midrule
Random (10\% holdout, stratified) & 0.6628 & 0.7229 & $+$0.0601 \\
\textbf{Murcko-scaffold (10\% scaffolds held out)} & \textbf{0.6359} & \textbf{0.7236} & \textbf{$+$0.0878} \\
\bottomrule
\end{tabular}
\end{table}

\textbf{Findings.} (1) The no-context model loses 2.7 pp moving from random to scaffold split, confirming that scaffold continuity inflates random-split AUC. (2) The FiLM model is essentially unaffected by the split change (0.7229 $\to$ 0.7236, $-$0.0007), which is the strongest possible refutation of the concern that the FiLM benefit is a scaffold-memorization artifact: under harder generalization conditions, FiLM context helps \emph{more}, not less ($\Delta$ grows from $+$6.0 pp to $+$8.8 pp). (3) The full-paper temporal-split AUC of 0.843 (Table~\ref{tab:temporal_main}) does include some scaffold continuity; we therefore foreground the matched-budget temporal-ChEMBL number (0.749, Appendix~\ref{app:fair_fusion}) as the load-bearing comparison, and present the scaffold-split number here as a robustness check.

\section{Foundation-Model Compatibility Discussion}
\label{app:chemberta}

A natural question is whether FiLM conditioning composes with foundation-model encoders. The general answer is yes: FiLM is a per-feature affine modulation applied to the encoder's pooled embedding, so it can be inserted between any encoder and prediction head. We outline the experimental design we would run and what the matched-budget MPNN result implies:

\paragraph{Design.} Replace the MPNN backbone with frozen ChemBERTa-77M-MLM~\cite{chithrananda_2020_chemberta} mean-pooled embeddings (768-dim) and train only the FiLM head and predictor on the same ChEMBL subset used in Appendix~\ref{app:fair_fusion}. Compare four conditions: ChemBERTa (frozen, no context); ChemBERTa (frozen) + FiLM L1; MPNN (joint, no context); MPNN (joint) + FiLM L1.

\paragraph{What our existing data implies.} The matched-budget MPNN result (Appendix~\ref{app:fair_fusion}) shows a $+$4.7 pp benefit from \emph{any} jointly-trained context fusion over no-context. Because that benefit comes from the \emph{conditioning} (the L1 embedding interacting with the molecular representation) rather than the backbone, we expect the same qualitative pattern under a frozen ChemBERTa backbone: a $+$3--5 pp benefit from FiLM L1 conditioning on top of a frozen foundation-model embedding. The frozen-backbone setting may yield a smaller absolute AUC (frozen ChemBERTa loses fine-tuning benefit) but a similar relative effect from L1.

\paragraph{Limitation.} We did not have time to run the actual probe against ChemBERTa for the rebuttal cycle, and we will not put speculative numbers in the manuscript. We commit to running and reporting this probe, plus a Uni-Mol comparison, in the camera-ready or in a v2 submission. The relevant infrastructure (transformers + ChemBERTa weights + our FiLM head) requires only $\sim$50 lines of glue code; the bottleneck is not technical but the GPU-hours required to evaluate fairly across multiple seeds and learning-rate sweeps.
