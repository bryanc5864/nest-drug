...
\usepackage{amsthm}
% \usepackage{algorithm}        % unavailable on minimal LaTeX installs
% \usepackage{algorithmic}      % unavailable on minimal LaTeX installs
...
Reviewers correctly note that DUD-E--ChEMBL leakage (mean 50\% of actives) could confound any claim that ``context helps,'' since the L1 benefit could simply reflect the model retrieving training-set neighbors. We test this directly by stratifying the L1 benefit on per-target leakage fraction (Table~\ref{tab:leakage}).
...
\begin{tabular}{@{}lccc@{}}
\toprule
Stratum & Targets & Mean $\Delta$ AUC & 1-sample $p$ ($\Delta = 0$) \\
\midrule
...
Reviewer concerns about the absence of \emph{actionable} guidance are well-taken: ``context helps when distributions align'' is post-hoc. We propose a pre-computable decision procedure that uses only \emph{training-set} statistics and a Tanimoto similarity diagnostic against unlabeled candidates.
...
This appendix provides the full setup and per-fusion numbers behind Table~\ref{tab:fusion} (matched-budget regime), which addresses the concern (raised by all four reviewers) that the original 24.2 pp ``FiLM $>$ concatenation'' gap was inflated by an underspecified concatenation baseline.
...
\paragraph{Results.} Mean validation AUC and RMSE across 2 seeds (full results from \texttt{/tmp/nest\_rebuttal/fair\_concat\_full.json}):
