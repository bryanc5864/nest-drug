...
\titlerunning{When Does Context Help?}
...
\keywords{molecular property prediction \and graph neural networks \and context conditioning \and drug discovery \and virtual screening \and FiLM \and multi-task learning}
\end{abstract}
...
\textbf{DUD-E Benchmark Limitations.} We identify three critical issues:
...
\end{itemize}
These limitations mean DUD-E absolute numbers should be interpreted cautiously. We provide temporal split evaluation (Sect.~\ref{sec:temporal}) as more rigorous evidence.
...
\begin{table}[t]
\caption{L1 context ablation on DUD-E (5 seeds, paired $t$-test with Bonferroni correction). ``Generic L1'' uses zero embedding at inference; ``Correct L1'' uses target-specific learned embedding. $\Delta$ = Correct $-$ Generic.}
...
\begin{figure}[t]
\centering
\includegraphics[width=0.95\textwidth]{figures/fig3_attribution.pdf}
...
\textbf{When to Use Context Conditioning.}
...
\textbf{What L1 Embeddings Learn.}
...
\textbf{Benchmark Recommendations.}
...
We conducted ablation studies on L2 (assay) and L3 (round) context levels. Neither showed significant effects in our experiments (Fig.~\ref{fig:appendix}):

\begin{figure}[t]
\centering
\includegraphics[width=0.95\textwidth]{figures/fig_appendix.pdf}
...
\textbf{Molecular Featurization.} We convert SMILES strings to molecular graphs using RDKit. Atom features (70-dim) include:
...
\textbf{Activity Value Processing.} We convert IC$_{50}$/K$_i$ values to pIC$_{50}$/pK$_i$ via $-\log_{10}(\text{value in M})$. Values are clipped to $[3, 12]$ and standardized per-target.

\subsection{Training Details}

\textbf{Pretraining.} We pretrain on ChEMBL 35 for 50 epochs with batch size 256, learning rate $3 \times 10^{-4}$, and cosine annealing. Pretraining takes approximately 120 GPU-hours on A100.

\textbf{Fine-tuning.} We fine-tune on DUD-E targets for 100 epochs with differential learning rates:
\begin{itemize}[noitemsep]
...
\textbf{Early Stopping.} We use patience of 20 epochs based on validation ROC-AUC.

\subsection{Evaluation Protocol}

\textbf{Cross-Validation.} We use 5-fold stratified cross-validation, maintaining active/decoy ratios across folds. Final metrics are reported as mean $\pm$ standard error across folds and 5 random seeds (25 total runs per experiment).

\textbf{Statistical Testing.} We use paired $t$-tests with Bonferroni correction for multiple comparisons. Effect sizes are reported as Cohen's $d$.

\section{DUD-E Benchmark Analysis}
\label{app:dude_analysis}

We provide additional analysis of DUD-E benchmark characteristics, addressing known limitations in the literature.
...
\begin{tabular}{@{}lrrcc@{}}
\toprule
Target & Train Act. & Per-Target RF & \methodname{} & Winner \\
\midrule
...
\toprule
Target & ROC-AUC & PR-AUC & EF@1\% & EF@5\% & Sens. & Spec. \\
\midrule
EGFR & $.899 \pm .012$ & $.412 \pm .031$ & $18.2 \pm 2.1$ & $8.4 \pm 0.9$ & $.821$ & $.892$ \\
DRD2 & $.934 \pm .008$ & $.523 \pm .028$ & $22.1 \pm 1.8$ & $9.7 \pm 0.7$ & $.867$ & $.923$ \\
ADRB2 & $.763 \pm .019$ & $.298 \pm .042$ & $12.4 \pm 2.4$ & $5.8 \pm 1.1$ & $.712$ & $.789$ \\
BACE1 & $.842 \pm .015$ & $.367 \pm .035$ & $15.6 \pm 2.0$ & $7.2 \pm 0.8$ & $.789$ & $.856$ \\
ESR1 & $.817 \pm .014$ & $.341 \pm .038$ & $14.3 \pm 2.2$ & $6.5 \pm 0.9$ & $.756$ & $.834$ \\
HDAC2 & $.901 \pm .011$ & $.428 \pm .030$ & $19.1 \pm 1.9$ & $8.8 \pm 0.8$ & $.834$ & $.898$ \\
JAK2 & $.862 \pm .013$ & $.378 \pm .033$ & $16.2 \pm 2.1$ & $7.5 \pm 0.9$ & $.798$ & $.867$ \\
PPARG & $.748 \pm .021$ & $.287 \pm .045$ & $11.8 \pm 2.5$ & $5.4 \pm 1.2$ & $.698$ & $.776$ \\
CYP3A4 & $.782 \pm .018$ & $.312 \pm .041$ & $13.5 \pm 2.3$ & $6.1 \pm 1.0$ & $.734$ & $.802$ \\
FXA & $.846 \pm .014$ & $.371 \pm .034$ & $15.9 \pm 2.0$ & $7.3 \pm 0.8$ & $.792$ & $.859$ \\
\midrule
\textbf{Mean} & $.839 \pm .006$ & $.372 \pm .014$ & $15.9 \pm 0.8$ & $7.3 \pm 0.4$ & $.780$ & $.850$ \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{tabular}{@{}lcccccc@{}}
\toprule
Target & Rounds & \multicolumn{2}{c}{Hit Rate (\%)} & Enrich. & \multicolumn{2}{c}{Expts to 50} \\
\cmidrule(lr){3-4} \cmidrule(lr){6-7}
 & & Rand & Model & & Rand & Model \\
\midrule
...
\begin{tabular}{@{}lcccc@{}}
\toprule
Target & 1-NN AUC & Act-Act Sim & Dec-Act Sim & Gap \\
\midrule
...
\begin{tabular}{@{}lcccc@{}}
\toprule
Target & DUD-E Act. & ChEMBL & Act. Leak & Dec. Leak \\
\midrule
...
\section{ESM-2 Protein Embedding Analysis}
\label{app:esm2}
...
\textbf{Positive Impacts.} \methodname{} could accelerate drug discovery by reducing experimental burden, potentially leading to faster development of therapeutics for unmet medical needs. The context-conditional approach may improve model reliability in real pharmaceutical settings where temporal shift is ubiquitous.

\textbf{Potential Risks.} Activity prediction models could theoretically be misused to identify toxic compounds. We mitigate this by training only on therapeutic targets and not releasing models for toxicity prediction. The model may perpetuate biases in training data, potentially overlooking promising compounds for underrepresented target classes.

\textbf{Environmental Considerations.} Training required approximately 200 GPU-hours on A100 hardware, corresponding to roughly 30 kg CO$_2$eq. We release pretrained models to avoid redundant computation.
